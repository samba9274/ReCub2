...
class ToolTip( Toplevel ):
	...
	@staticmethod
	def LoadToolTips ( filepath ):	# Perhaps allow a reload of tips?
		try:
			if filepath is None:
				tipsFile = open("Tooltips.txt", "r")
			else:
				tipsFile = open(filepath, "r")
			if tipsFile:
	...
	def show( self ):
		...
		if ToolTip.ShowTipNumber is True and self.TipID is not None:
			...
			text = text.replace(self.TipNumText,"")
